...
- Externe Nutzung: nur `/api/sentero/exchange/v1/*`.
...
2. Reverse-Proxy oder Router veroeffentlicht extern nur `/api/sentero/exchange/v1/*`.
...
server {
    ...
    location /api/sentero/exchange/v1/ {
...
```caddyfile
aal.example.org {
    handle /api/sentero/exchange/v1/* {
