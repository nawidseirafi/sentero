...
class MqttSensorSourceTests(unittest.TestCase):
            ...
            FakeMessage("zigbee2mqtt/bridge/devices", [
                {
                    ...
                    "definition": {
                        "model": "ZG-204ZH",
                        "vendor": "HOBEIAN",
                        "exposes": [{"type": "binary", "name": "occupancy", "property": "occupancy"}],
                    },
                }
            ])
        ])
    ...
    def test_zigbee_presence_role_exposes_environment_metrics(self) -> None:
        ieee = "0xa4c1389a3e0a13e3"
        mqtt = SnapshotMqtt([
            FakeMessage(f"zigbee2mqtt/{ieee}", {
                "occupancy": False,
                "battery": 90,
                "temperature": 22.6,
                "humidity": 48,
        ...
        with tempfile.TemporaryDirectory() as tmpdir, patch.dict(
            ...
            mapping.upsert_role({
                "role": "living_room_presence",
                "room": "living_room",
                "entity_id": f"binary_sensor.{ieee}",
                "device_id": ieee,
                "friendly_name": "Wohnzimmer Präsenz",
                "device_class": "occupancy",
                "domain": "binary_sensor",
    ...
    def test_environment_metrics_ignore_configuration_entities(self) -> None:
        ...
        states = [
            {"entity_id": "number.0xa4c1389a3e0a13e3_temperature_calibration", "domain": "number", "state": "0", "device_id": "0xa4c1389a3e0a13e3"},
            {"entity_id": "number.0xa4c1389a3e0a13e3_humidity_calibration", "domain": "number", "state": "0", "device_id": "0xa4c1389a3e0a13e3"},
            {"entity_id": "number.0xa4c1389a3e0a13e3_illuminance_interval", "domain": "number", "state": "3", "device_id": "0xa4c1389a3e0a13e3"},
            {"entity_id": "sensor.0xa4c1389a3e0a13e3_temperature", "domain": "sensor", "state": "22.9", "device_class": "temperature", "device_id": "0xa4c1389a3e0a13e3"},
            {"entity_id": "sensor.0xa4c1389a3e0a13e3_humidity", "domain": "sensor", "state": "56", "device_class": "humidity", "device_id": "0xa4c1389a3e0a13e3"},
            {"entity_id": "sensor.0xa4c1389a3e0a13e3_illuminance", "domain": "sensor", "state": "1104", "device_class": "illuminance", "device_id": "0xa4c1389a3e0a13e3"},
        ]
        metrics = environmental_metrics_from_state(role, states)
    ...
    def test_delete_zigbee2mqtt_sensor_uses_ieee_when_homeassistant_name_differs(self) -> None:
        mqtt = SnapshotMqtt([])
        ieee = "0xa4c1389a3e0a13e3"
        with tempfile.TemporaryDirectory() as tmpdir, patch.dict(
            os.environ,
            ...
            mapping.upsert_role({
                "role": "toilet_presence",
                "room": "toilet",
                "entity_id": f"binary_sensor.{ieee}_presence",
                "device_id": "ha-device-id",
                "friendly_name": "Toilette Präsenz",
                "device_class": "presence",
                "domain": "binary_sensor",
                "source": "zigbee2mqtt",
                "confidence": 100,
    ...
    def test_delete_mqtt_mapping_with_ieee_uses_zigbee_remove_not_factory_reset(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir, patch.dict(
            ...
            mapping.sensor_source.mqtt = mqtt
            mapping.upsert_role({
                "role": "office_presence",
                "room": "office",
                "entity_id": f"binary_sensor.{ieee}_presence",
                "device_id": ieee,
                "friendly_name": "Arbeitszimmer Präsenz",
                "device_class": "presence",
                "domain": "binary_sensor",
                "source": "mqtt",
                "confidence": 100,
    ...
    def test_mqtt_uuid_presence_ignores_old_retained_c1001_state(self) -> None:
        ...
        mqtt = SnapshotMqtt([
            *esp32_presence_messages(old_device_id, availability="offline"),
            *esp32_presence_messages(device_id, availability="online"),
        ])
        with tempfile.TemporaryDirectory() as tmpdir, patch.dict(os.environ, {"SENTERO_SENSOR_SOURCE": "mqtt"}, clear=False):
            mapping = DeviceMappingService(database_path=Path(tmpdir) / "sentero.db")
            mapping.mqtt = mqtt
    ...
    def test_homeassistant_zigbee_pairing_does_not_offer_existing_unassigned_sensor(self) -> None:
        class MutableHomeAssistant(FakeHomeAssistant):
            ...
            def get_states(self) -> list[dict]:
                ...
                return [{
                    "entity_id": "binary_sensor.guest_wc_presence_sensor_occupancy",
                    "state": self.state,
                    ...
                    "attributes": {
                        "friendly_name": "Guest WC Presence Sensor Belegung",
                        "device_class": "occupancy",
                        "device_id": "ha-existing-presence-device",
                    },
                }]

        ha = MutableHomeAssistant()
        with tempfile.TemporaryDirectory() as tmpdir, patch.dict(os.environ, {"SENTERO_SENSOR_SOURCE": "homeassistant"}, clear=False):
            service = DeviceMappingService(database_path=Path(tmpdir) / "sentero.db", ha=ha)
            service.mqtt = FakeMqtt()
            started = service.start_zigbee_pairing("hallway_presence", "hallway", duration=60)
            ha.state = "on"
            found = service.candidates(started["session_id"], dev=True)

            with self.assertRaises(ValueError):
                service.confirm(started["session_id"], "binary_sensor.guest_wc_presence_sensor_occupancy", name="Flur Präsenz", room="hallway")

        self.assertEqual(found["status"], "waiting_for_signal")
        self.assertIsNone(found["candidate"])
